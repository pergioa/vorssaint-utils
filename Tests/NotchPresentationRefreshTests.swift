...
enum NotchPresentationRefreshContract {
    ...
    static func run(_ suite: TestSuite) {
        ...
        suite.expect(invalidations == 4, "each mode change publishes its new presentation size")
        for _ in 0..<1000 { service.refreshPresentation() }
        suite.expect(invalidations == 4, "unchanged presentations do not repeatedly invalidate SwiftUI layout")
