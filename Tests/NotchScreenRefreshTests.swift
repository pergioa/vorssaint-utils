...
enum NotchScreenRefreshContract {
    ...
    static func run(_ suite: TestSuite) {
        ...
        suite.expect(simulated.geometry.compactSideRoom == nil && simulated.menuSpaceGeneration > beforeChange
        ...
        simulated.syncMenuSpaceMonitoring()
        suite.expect(simulated.geometry.compactSideRoom == nil,
               "another preference sync cannot restore a previous app's menu clearance")
        simulated.geometry.compactSideRoom = 64
        simulated.invalidateMenuSpace()
        suite.expect(simulated.geometry.compactSideRoom == 64,
