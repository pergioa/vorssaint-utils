...
enum NotchMusicHardeningTests {
    ...
    private static func sourcePriority(_ suite: TestSuite) {
        ...
        suite.expect(choose([browser, paused], previous: 10) == paused, "pausing music keeps its resume control reachable")
        suite.expect(choose([browser, paused]) == paused, "reopening the music surface can still reach paused music")
    ...
    private static func controlLifecycle(_ suite: TestSuite) {
        ...
        service.stop()
        Contract.DispatchQueue.main.drain()
        suite.expect(service.launches == cancelledLaunches && !service.wantsPlayback,
